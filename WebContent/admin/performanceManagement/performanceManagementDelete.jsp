...
<!-- @author 宋杨
  ...
  -- @notes tname对应表名称
 -->
<script>var tname="performance";</script>
<%String tname="performance";
...
function deleteRow(obj){
	var tableName=tname;
	var clumnName = document.getElementById('clumn1').innerHTML;
	var clumnValue=obj.id;
	var cof=window.confirm("确认删除吗?");
	if(cof==true){
	　　  $.ajax({
	         type:"post",//请求方式
	         contentType: "application/x-www-form-urlencoded; charset=utf-8",
	         url:"${basePath}/controller/deleteController.jsp",//请求地址
	         data:{"tableName":tableName,"clumnName":clumnName,"clumnValue":clumnValue},//传递给controller的json数据
...
<%for(int i=1;i<=countCol;i++){
    ...
    <td id='<%="result"+String.valueOf(t)%>'><%=(String)request.getAttribute("result"+String.valueOf(t))%></td>
     <%}%>
     <th><a id='<%=(String)request.getAttribute("result"+String.valueOf((j-1)*countCol+1))%>' class="delete"  onclick="deleteRow(this)">删除</a></th>
</tr>
<%}%>
</table></div>
<%}%>
